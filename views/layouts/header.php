...
<body class="bg-slate-50 text-slate-800 font-sans h-screen flex overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col shadow-lg z-20">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            ...
            $activeClass = "bg-slate-800 border-l-4 border-teal-400 text-white";
            $inactiveClass = "border-l-4 border-transparent text-slate-300 hover:bg-slate-800 hover:text-white group";
            $activeIcon = "text-teal-400";
            $inactiveIcon = "text-slate-400 group-hover:text-teal-400 transition-colors";
            ?>
            <ul class="space-y-1">
                <li>
                    <a href="<?= $baseRoute ?>/" class="flex items-center px-6 py-2 transition-all <?= $route === '' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === '' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                        Home
                    </a>
                </li>
                <li>
                    <a href="<?= $baseRoute ?>/tasks" class="flex items-center px-6 py-2 transition-all <?= $route === 'tasks' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === 'tasks' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        My Tasks
                    </a>
                </li>
                <li>
                    <a href="<?= $baseRoute ?>/inbox" class="flex items-center px-6 py-2 transition-all <?= $route === 'inbox' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === 'inbox' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path></svg>
                        Inbox
                        <?php 
                ...
                <li>
                    <a href="<?= $baseRoute ?>/reports" class="flex items-center px-6 py-2 transition-all <?= $route === 'reports' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === 'reports' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                        Reporting
                    </a>
                </li>
                <li>
                    <a href="<?= $baseRoute ?>/teams" class="flex items-center px-6 py-2 transition-all <?= $route === 'teams' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === 'teams' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        Teams
                    </a>
                </li>
                <li>
                    <a href="<?= $baseRoute ?>/projects" class="flex items-center px-6 py-2 transition-all <?= $route === 'projects' || $route === 'project' || $route === 'project_settings' ? $activeClass : $inactiveClass ?>">
                        <svg class="w-5 h-5 mr-3 <?= $route === 'projects' || $route === 'project' || $route === 'project_settings' ? $activeIcon : $inactiveIcon ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                        Projects
                    </a>
                </li>
            </ul>
            ...
            <ul class="space-y-1 max-h-64 overflow-y-auto overflow-x-hidden" style="scrollbar-width: thin; scrollbar-color: #475569 transparent;">
                ...
                try {
                    ...
                    foreach ($recentProjects as $rp) {
                        ...
                        $isActiveProject = ($route === 'project' || $route === 'project_settings') && $currentProjectId == $rp['id'];
                        $projectRowClass = $isActiveProject ? "bg-slate-800" : "hover:bg-slate-800";
                        $projectTextClass = $isActiveProject ? "text-white font-medium" : "text-slate-300 hover:text-white";
                ?>
                <li>
                    <div class="flex items-center justify-between px-6 py-2 group text-sm transition-colors <?= $projectRowClass ?>">
                        <a href="<?= $baseRoute ?>/project?id=<?= $rp['id'] ?>" class="flex items-center flex-1 <?= $projectTextClass ?>">
                            <span class="w-2 h-2 rounded-full <?= $circleColor ?> mr-3 <?= $isActiveProject ? 'ring-2 ring-slate-600' : '' ?>"></span>
                            <?= htmlspecialchars($rp['name']) ?>
                        </a>
                        <a href="<?= $baseRoute ?>/project_settings?id=<?= $rp['id'] ?>" class="text-slate-500 hover:text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity" title="Project Settings">
        ...
        <div class="p-4 border-t border-white/10 group cursor-pointer relative">
            ...
            <div class="absolute bottom-full left-0 w-full p-2 pb-4 hidden group-hover:block transition-all z-50">
                <div class="bg-slate-800 rounded-lg shadow-lg border border-slate-700 py-1">
                    <a href="<?= $baseRoute ?>/settings" class="block px-4 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-700 flex items-center">
                    ...
                    <a href="<?= $baseRoute ?>/logout" class="block px-4 py-2 text-sm text-rose-400 hover:text-rose-300 hover:bg-slate-700 flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                        Logout
                    </a>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <div id="global-notifications-dropdown" class="hidden absolute top-full right-0 mt-2 w-80 bg-white border border-slate-200 shadow-xl rounded-xl flex flex-col transform origin-top-right z-[100] max-h-[85vh]">
                        ...
                        <div class="overflow-y-auto w-full p-2 space-y-1 bg-white">
                            ...
                            <a href="<?= $rn['task_id'] ? $baseRoute . '/tasks?id='.$rn['task_id'] : $baseRoute . '/inbox' ?>" class="block px-3 py-2.5 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors relative border border-transparent hover:border-slate-200">
                                <?php if (!$rn['is_read']): ?>
                        ...
                        <div class="px-4 py-2 border-t border-slate-100 text-center bg-slate-50/50 rounded-b-xl">
                            <a href="<?= $baseRoute ?>/inbox" class="text-xs font-semibold text-teal-600 hover:text-teal-800 transition-colors">Open Inbox</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative group">
                    ...
                    <div class="absolute right-0 top-full pt-2 w-48 hidden group-hover:block z-50">
                        <div class="bg-white border border-slate-200 shadow-xl rounded-xl overflow-hidden flex flex-col transform origin-top-right">
                            <a href="<?= $baseRoute ?>/settings" class="px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 transition-colors">Settings</a>
                            <a href="<?= $baseRoute ?>/logout" class="px-4 py-2 text-sm text-rose-600 hover:bg-slate-50 transition-colors">Logout</a>
                        </div>
                    </div>
                </div>
            </div>
